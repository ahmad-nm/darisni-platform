        ...
        <div className={style.container}>
          ...
          <div className={style.header}>
            <div className={style.headerContent}>
              <div>
                <h1 className={style.title}>Course Suggestions</h1>
                <p className={style.subtitle}>
                  View and manage course suggestions submitted by users.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className={style.statsContainer}>
            <div className={style.statCard}>
              ...
              <span className={style.statLabel}>Total Suggestions</span>
            </div>
            <div className={style.statCard}>
              <span className={style.statNumber}>{uniquePhones}</span>
              <span className={style.statLabel}>Unique Users</span>
            </div>
          </div>
          ...
          {filtered.length === 0 ? (
            ...
            <div className={style.suggestionsGrid}>
              {filtered.map(s => (
                <div key={s.id} className={style.suggestionCard}>
                  <div className={style.cardHeader}>
                    <span className={style.userName}>{s.user_name}</span>
                    <span className={style.phone}>{s.phone}</span>
                  </div>
            ...
            <div className={style.modalOverlay} onClick={closeModal}>
              <div className={style.modalContent} onClick={e => e.stopPropagation()}>
                ...
                <div className={style.modalBody}>
                  <div><strong>Name:</strong> {selected.user_name}</div>
                  <div><strong>Phone:</strong> {selected.phone}</div>
                  <div><strong>Suggestion:</strong></div>
                  <div style={{margin: '12px 0', whiteSpace: 'pre-wrap'}}>{selected.suggestion}</div>
                  <div><strong>Submitted:</strong> {new Date(selected.created_at).toLocaleString()}</div>
                </div>
                <div className={style.modalActions}>
                  <button className={style.deleteButton} onClick={() => handleDelete(selected.id)}>
                    🗑️ Delete
                  </button>
                  <button className={style.closeViewButton} onClick={closeModal}>
                    Close
                  </button>
                </div>
              </div>

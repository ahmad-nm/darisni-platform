...
export default function ReviewManagement({ reviews: initialReviews, stats: initialStats }) {
    ...
    useEffect(() => {
        ...
        if (searchTerm) {
            filtered = filtered.filter(review => 
                ...
                review.subject?.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const getFilterLabel = (filterType) => {
        switch (filterType) {
            case 'course': return 'Course Reviews';
            case 'tutor': return 'Tutor Reviews';
            case 'darisni': return 'Platform Reviews';
                ...
                <div className={styles.container}>
                    <div className={styles.header}>
                        <div className={styles.headerContent}>
                            <div>
                                ...
                                <p className={styles.subtitle}>Manage all platform reviews and ratings</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className={styles.statsContainer}>
                        <div className={styles.statCard}>
                            ...
                            <div className={styles.statInfo}>
                                <span className={styles.statNumber}>{stats.total_reviews}</span>
                        ...
                        <div className={styles.statCard}>
                            <div className={styles.statIcon}>📚</div>
                            <div className={styles.statInfo}>
                                <span className={styles.statNumber}>{stats.course_reviews}</span>
                                <span className={styles.statLabel}>Course Reviews</span>
                            </div>
                        </div>
                        <div className={styles.statCard}>
                            <div className={styles.statIcon}>👨‍🏫</div>
                            <div className={styles.statInfo}>
                                <span className={styles.statNumber}>{stats.tutor_reviews}</span>
                                <span className={styles.statLabel}>Tutor Reviews</span>
                            </div>
                        </div>
                        <div className={styles.statCard}>
                            <div className={styles.statIcon}>🏢</div>
                            <div className={styles.statInfo}>
                                <span className={styles.statNumber}>{stats.darisni_reviews}</span>
                                <span className={styles.statLabel}>Platform Reviews</span>
                            </div>
                        </div>
                        <div className={styles.statCard}>
                            <div className={styles.statIcon}>⭐</div>
                            <div className={styles.statInfo}>
                                <span className={styles.statNumber}>{stats.average_ratings?.overall || 0}</span>
                        ...
                        <div className={styles.statCard}>
                            <div className={styles.statIcon}>🆕</div>
                            <div className={styles.statInfo}>
                                <span className={styles.statNumber}>{stats.recent_reviews?.total || 0}</span>
                                <span className={styles.statLabel}>Recent (7 days)</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <div className={styles.filterContainer}>
                            <div className={styles.filterButtons}>
                                ...
                                <button
                                    ...
                                    onClick={() => handleFilterChange('darisni')}
                                >
                                    🏢 Platform
                                </button>
                            </div>
                    ...
                    {error && (
                        <div className={styles.errorMessage}>
                            <p>{error}</p>
                            <button onClick={fetchReviews} className={styles.retryBtn}>
                                Retry
                            </button>
                        </div>
                    )}
                    ...
                    <div className={styles.reviewsContainer}>
                        <div className={styles.resultsHeader}>
                            <h3>{getFilterLabel(filter)} ({filteredReviews.length})</h3>
                        </div>
                        ...
                        {currentReviews.length === 0 ? (
                            <div className={styles.emptyState}>
                                ...
                                <p>No reviews match your current filter and search criteria.</p>
                            </div>
                        ) : (
                            <div className={styles.reviewsGrid}>
                                {currentReviews.map(review => (
                                    <div key={`${review.type}-${review.id}`} className={styles.reviewCard}>
                                        <div className={styles.reviewHeader}>
                                            <div className={styles.reviewType}>
                                                <span 
                                                    className={styles.typeIcon}
                                                    style={{ backgroundColor: getReviewTypeColor(review.type) }}
                                                >
                    ...
                    <div className={styles.reviewPreview}>
                        <div className={styles.previewHeader}>
                            ...
                            <div className={styles.previewRating}>
                                {[...Array(5)].map((_, index) => (
                                    <span key={index} className={index < review.rating ? styles.starFilled : styles.starEmpty}>
                                        ⭐
                                    </span>
                                ))}
                            </div>
                        ...
                        <div className={styles.previewContent}>
                            ...
                            <p className={styles.previewUser}>By: {review.user?.name}</p>
                            {review.feedback && (
                                <p className={styles.previewFeedback}>"{review.feedback}"</p>
                            )}
                        </div>
                    </div>

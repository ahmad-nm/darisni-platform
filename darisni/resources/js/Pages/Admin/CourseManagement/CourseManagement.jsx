...
export default function CourseManagement({ courses: initialCourses, categories: initialCategories, tutors: initialTutors, stats: initialStats }) {
    ...
    const deleteCourse = async (courseId) => {
        return new Promise((resolve, reject) => {
            router.delete(`/admin/courses/${courseId}`, {
                ...
                onError: (errors) => {
                    reject({ response: { data: { message: 'Failed to delete course' } } });
                }
            });
        });
    };
    ...
    const getCourseChapters = async (courseId) => {
        try {
            const response = await fetch(`/admin/courses/${courseId}/chapters`, {
                method: 'GET',
                headers: {
    ...
    const getCategoryName = (categoryId) => {
        if (!categoryId || !Array.isArray(categories) || categories.length === 0) return 'Unknown';
        const category = categories.find(cat => Number(cat.id) === Number(categoryId));
        return category ? category.name : 'Unknown';
    };

    const getTutorName = (tutorId) => {
        if (!tutorId || !Array.isArray(tutors) || tutors.length === 0) return 'Unknown';
                ...
                <div className={styles.container}>
                    <div className={styles.header}>
                        <div className={styles.headerContent}>
                            <div>
                                ...
                                <p className={styles.subtitle}>Manage all courses on your platform</p>
                            </div>
                            <button 
                    ...
                    <div className={styles.statsContainer}>
                        <div className={styles.statItem}>
                            <span className={styles.statNumber}>{filteredCourses.length}</span>
                        ...
                        <div className={styles.statItem}>
                            <span className={styles.statNumber}>{tutors.length}</span>
                            <span className={styles.statLabel}>Tutors</span>
                        </div>
                        <div className={styles.statItem}>
                            <span className={styles.statNumber}>
                                {filteredCourses.filter(c => c.price === 0 || c.price === null).length}
                            </span>
                        ...
                        {currentCourses.map(course => (
                            ...
                            >                
                                ...
                                <div className={styles.courseContent}>
                                    ...
                                    <p className={styles.courseCategory}>{getCategoryName(course.category_id)}</p>
                                    <p className={styles.courseTutor}>👨‍🏫 {getTutorName(course.tutor_id)}</p>
                                    
                                    <div className={styles.courseDetails}>
                                        {course.subject && (
                                            <span className={styles.courseDetail}>📖 {course.subject}</span>
                                        )}
                                        {course.semester && (
                                            <span className={styles.courseDetail}>📅 Sem {course.semester}</span>
                                        )}
                                        {course.lectures && (
                                            <span className={styles.courseDetail}>🎥 {course.lectures} lectures</span>
                                        )}
                                    </div>
...
function ChapterManagementModal({ 
    ...
    const handleSaveChapter = async (e) => {
        ...
        try {
            if (editingChapter) {
                ...
                const updatedChapter = await updateCourseChapter(editingChapter.id, {
                    course_id: course.id,
    ...
    return (
        <div className={styles.modalOverlay} onClick={onClose}>
            <div className={styles.chapterModal} onClick={e => e.stopPropagation()}>
                                ...
                                <div key={chapter.id} className={styles.chapterItem}>
                                    ...
                                    <div className={styles.chapterContent}>
                                        ...
                                        <div className={styles.chapterMeta}>
                                            <span>Created: {chapter.created_at ? new Date(chapter.created_at).toLocaleDateString() : 'Unknown'}</span>
...
function CourseModal({ course, mode, categories, tutors, onClose, onSave, setModalMode }) {
                ...
                {mode === 'view' ? (
                    <div className={styles.courseDetailsView}>
                        <div className={styles.courseImageSection}>
                            {course.image ? (
                                <img src={course.image} alt={course.title} className={styles.courseImageLarge} />
                            ) : (
                        ...
                        <div className={styles.courseInfoGrid}>
                            ...
                            <div className={styles.infoItem}>
                                ...
                                <span>{getCategoryName(course.category_id)}</span>
                            </div>
                        ...
                        <div className={styles.formRow}>
                            <div className={styles.formGroup}>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setFormData(prev => ({ ...prev, lectures: e.target.value }))}
                                    className={styles.modalInput}
                                />
                            </div>
                            ...
                            <div className={styles.formGroup}>
                                ...
                                {formData.image && (
                                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        <img src={formData.image} alt="Course" style={{ maxWidth: 80, marginTop: 8 }} />
                                        <button
                                            type="button"
            ...
            <div className={styles.deleteModal} onClick={e => e.stopPropagation()}>
                ...
                <div className={styles.deleteModalContent}>
                    ...
                    <div className={styles.coursePreview}>
                        ...
                        <span>Code: {course.code}</span><br />
                        <span>Price: {course.price ? `$${course.price}` : 'Free'}</span>
                    </div>
